...
TRAINING_MODE <- "full" # Change to "full" for actual training
    ...
    while (!env$done && step_count < CONFIG$max_steps_per_episode) {
      ...
      if (replay_buffer$size() >= CONFIG$warmup_steps) {
        ...
        if (!is.null(batch)) {
          ...
          current_q_py <- predict(online_net, batch$states, verbose = 0)
          current_q <- as.matrix(current_q_py) # Ensure it's a standard R matrix
          for (i in 1:length(batch$actions)) {
            current_q[i, batch$actions[i] + 1] <- targets[i]
          }

          # Create tensors directly using tf$constant for robust type handling
          states_tensor <- tf$constant(batch$states, dtype = "float32")
          q_tensor <- tf$constant(current_q, dtype = "float32")
          
          train_on_batch(online_net, states_tensor, q_tensor)
        }
      }
