...
# Authors: Ramneet Singth, Tayo Akinola, Omar Abdou
# Date: 2025-06-18
# Description: This R script addresses the first seven exercises from Task Sheet 1 of the Neural Networks in Finance seminar.


# Setup for Knitr (if this were an Rmd, these options would be in the setup chunk)
options(
  knitr.duplicate.label = "allow" # Allows duplicate chunk labels, useful when knitting
)
...
# install.packages("neuralnet") # For neural network implementation in Exercise 4
...
print(credit_transitions, 3)

# Verify rows sum to 1 (property of stochastic matrices)
cat("\nRow sums (should all equal 1.0):\n")
print(rowSums(credit_transitions))

# This example demonstrates the Markov property: a bond rated 'A' has the same probability distribution
# for next year's rating regardless of its rating history.
...
# According to our glossary, a **State** represents "all the information the agent sees right now when deciding."
# In our trading context, this is a window of recent returns that should satisfy the **Markov property** -
# containing sufficient information to predict what happens next.
...
visualize_state_vector <- function(return_data, t_index, window_size = 10) {
  ...
  ggplot(plot_data, aes(x = index, y = return_pct)) +
    ...
    geom_point(aes(color = point_type, size = point_type)) + # Use the new point_type variable
    scale_color_manual(
      name = "State",
...
# Our Q-network will approximate the optimal Q-values for each action given a specific state.
# The input to the network will be the state vector (e.g., 10 recent returns), and the output
# will be the Q-values for each possible action (e.g., Buy, Sell, Hold).
...
create_q_network <- function(input_nodes, hidden_nodes, output_nodes) {
  ...
  # For 'neuralnet', we typically define the formula dynamically.
  # Input variables will be 'X1', 'X2', ..., 'X_input_nodes'
  # Output variables will be 'Q_Buy', 'Q_Sell', 'Q_Hold' (or similar)
  
  # This function won't return a 'neuralnet' object directly because 'neuralnet'
  # requires data for its formula, which is part of the training step.
  # Instead, it sets up the parameters and acts as a conceptual factory.
  # The actual neural network training will use these parameters.
  
  # For demonstration, we'll create a dummy network structure (formula based).
  # In a real setup, we'd prepare data for training the network.
  
  # Example formula for neuralnet (conceptually for 3 outputs):
  # Q_Buy + Q_Sell + Q_Hold ~ X1 + X2 + ... + X_input_nodes
  
  # A dummy network function for prediction based on a simplified model
  # In a real RL setup, this would be the actual trained network.
  # For now, it simulates a network prediction based on random weights.
  predict_q_values <- function(state_vector, nn_model = NULL) {
    if (is.null(nn_model)) {
      ...
      # compute() returns a list, with $net.result containing the predictions
...
# In Q-learning, the agent learns by trying to match its predicted Q-values to a "target" Q-value.
# The target Q-value is typically calculated using the Bellman equation:
...
# For this exercise, we'll generate *artificial* targets. This means we're not running a full
# simulation to get actual rewards and next states. Instead, we'll define a simple rule
# for what good Q-values might look like for a given state. This simplifies the setup
# for understanding the concept without requiring a full environment simulation.
...
generate_artificial_q_target <- function(state_vector, action_taken, reward, discount_factor = 0.95) {
  # Input validation for state_vector
  ...
  # Simulate a "next state" by slightly perturbing the current state or a simple trend
  # This is a very simplistic simulation for *artificial* target generation
  # In a real environment, you would transition to the actual next state s'
  ...
  # Higher Q for Buy if recent returns were positive
  # Higher Q for Sell if recent returns were negative
  # Default Q for Hold
  ...
  # Add the immediate reward and a conceptual "future reward" component
  # In a real Q-learning, this "future reward" would be max Q(s', a') from the target network
  # Here, we'll just slightly adjust based on the immediate reward.
  # We are not calculating a true Bellman equation here as we don't have s_prime.
  # This is purely for generating plausible *artificial* targets.
...
# The Q-learning training loop is an iterative process where the agent interacts with the environment,
# collects experience, and uses that experience to update its Q-network.
...
#     This helps break correlations in sequence of experiences. (Conceptual here).
# 3.  **Action Selection**: Epsilon-greedy policy for exploration/exploitation.
# 4.  **Interact with Environment**: Take action, observe reward and next state. (Simulated here).
# 5.  **Sample Mini-batch**: From replay buffer.
# 6.  **Calculate Targets**: Using the Bellman equation (via the target network, conceptually here).
# 7.  **Train Network**: Update Q-network weights to minimize the difference between predicted Q and target Q.
# 8.  **Update Target Network**: Periodically copy Q-network weights to a separate target network. (Conceptual here).
...
train_q_network_simplified <- function(q_network_params, return_data,
  ...
  # Dummy training data preparation for 'neuralnet' (this is crucial for neuralnet)
  # neuralnet requires a data.frame where input and output variables are clearly defined
  # We'll collect a small set of (state, target_q) pairs for a conceptual 'training'.
  # In a real scenario, this would come from the replay buffer.
  
  # Initialize a dummy training data frame. We'll populate it with random-like data
  # to make neuralnet callable.
  # The formula will be 'Q_Buy + Q_Sell + Q_Hold ~ X1 + ... + X_window_size'
  ...
  # Collect some initial random data for training, to get a network object
  # In a real setup, this would be your replay buffer sampled data.
  ...
  # Initial (untrained) Q-network. neuralnet trains and returns the model.
  # Here we just initialize a simplified model.
  # Using rep(hidden_nodes, times=length(hidden_nodes)) for neuralnet's hidden argument
  # if hidden_nodes is a vector of layer sizes.
  ...
  # For simplicity, let's just use the result from the initial training of the dummy data
  # to represent our Q-network throughout this conceptual loop.
  # In reality, this model would be continuously retrained/updated.
  ...
  for (episode in 1:num_episodes) {
    ...
    for (step in 1:steps_per_episode) {
      ...
      # For simplicity, using basic min-max normalization based on historical range.
      # In a real system, you'd fit a scaler on training data and apply it consistently.
      ...
      # Handle case where min_ret == max_ret (no variation)
      if (is.nan(normalized_state_vector) || (max_ret - min_ret) == 0) {
        normalized_state_vector <- rep(0.5, length(current_state_vector)) # Default to middle if no variance
      }
      ...
      # For this conceptual loop, we'll assign a simple, simulated reward
      # based on the next day's return and the action taken.
      # This is NOT a full trading simulation, but a simplified reward logic.
      next_return <- spy_returns$return_pct[current_t_index + 1] # Reward based on next day's return
      ...
      # Cap reward to avoid extreme values in conceptual model
      reward <- min(max(reward, -1), 1)
      
      # 4. Get next state
      next_t_index <- current_t_index + 1
      
      # Check if we are at the end of the data (no next state available)
      ...
      if (!is_done) {
        ...
        normalized_next_state_vector <- rep(0, q_network_params$input_nodes) # Terminal state, Q_next is 0
      }
      
      # 5. Calculate Target Q-value using the Bellman equation
      # Q_target = Reward + Gamma * max(Q(s', a'))
      
      # Predict Q-values for the next state using the current Q-network (or a separate target network)
      if (!is_done) {
      ...
      # Get current Q-values for the current state to update only the action taken
      ...
      # All Q-values are kept as predicted, except for the action that was taken,
      # which is updated with the calculated target_q_for_action.
      ...
      # For neuralnet, this typically means creating new input/output pairs and retraining
      # or updating. In a true Deep Q-Network, you'd collect many (S, A, R, S', Done)
      # tuples in a replay buffer and then sample mini-batches for gradient descent.
      
      # For demonstration with 'neuralnet', we'll simulate a single update by creating
      # a tiny dataset for this step and symbolically showing the training.
      # In practice, `neuralnet` is not ideal for online, iterative updates of a Q-network
      # with a replay buffer; Keras/TensorFlow/PyTorch are built for this.
      ...
      # This part is highly simplified for 'neuralnet' demo.
      # A production DQL would use gradient descent frameworks.
      # We'll just print a message indicating a training step.
      if (step %% 50 == 0) { # Print update every 50 steps
        cat(paste0("  Step ", step, ": Action=", action_taken, ", Reward=", round(reward, 3), ", Predicted Q[", action_taken, "]=",
                   round(current_q_predictions[selected_action_idx], 3), ", Target Q=", round(target_q_for_action, 3), "\n"))
      }
      
      # For actually updating the neuralnet model, we would typically do:
      # q_network_model <- neuralnet(nn_formula, data = rbind(existing_training_data, training_data_point), ...)
      # This is computationally intensive for each step. A proper DQL uses mini-batch training.
      # Here, we *conceptually* update the network.
      # For simplicity, we just assume the network *would* be updated.
  ...
  # Return the last conceptual Q-network state for further use (if desired)
...
# Epsilon typically starts high (e.g., 1.0 for pure exploration) and decays over time to a small value (e.g., 0.01)
# to encourage more exploitation as the agent learns.
...
select_action_epsilon_greedy <- function(predict_q_function, state_vector, epsilon, q_network_model) {
  ...
  if (runif(1) < epsilon) {
    ...
    # cat("  [EXPLORE] Random action chosen.\n") # Uncomment for debugging exploration
  } else {
    # Exploit: Choose the action with the highest Q-value
    q_values <- predict_q_function(state_vector, q_network_model)
    action_idx <- which.max(q_values)
    # cat("  [EXPLOIT] Best action chosen.\n") # Uncomment for debugging exploitation
  }
  return(action_idx)
}
...
for (i in 1:5) {
  selected_action_idx <- select_action_epsilon_greedy(q_network_params$predict, normalized_test_state, epsilon_high, NULL) # Pass NULL for untrained model
...
for (i in 1:5) {
  selected_action_idx <- select_action_epsilon_greedy(q_network_params$predict, normalized_test_state, epsilon_low, NULL) # Pass NULL for untrained model
  cat(paste0("  Trial ", i, ": Selected Action = ", ACTIONS[selected_action_idx], "\n"))
}
...
# This will execute the conceptual training.
# Note: The 'neuralnet' package is not designed for iterative online learning from a replay buffer.
# The training here is highly conceptual. For actual DQL, Keras/TensorFlow or PyTorch are necessary.
# The returned `q_network_model` will be the result of a *single* `neuralnet` training call
# on the initial `dummy_train_data`, which represents a conceptual starting point.
# A true DQL updates weights incrementally.
...
final_q_network_model <- train_q_network_simplified(q_network_params, spy_returns,
                                                    num_episodes = 2, steps_per_episode = 100, # Reduced for quick demo
                                                    learning_rate = 0.01,
                                                    epsilon_start = 1.0, epsilon_end = 0.1, epsilon_decay = 0.9) # Faster decay for demo
